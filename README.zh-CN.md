...
截图交互与系统自带截图一致，唯一区别是：截图完成时不弹出右下角缩略图，而是直接把图片放进剪贴板。

拖拽框选，选区外变暗、尺寸徽标实时跟随，松开即完成截图。Esc 取消。

截图体验分为两种模式：

- 极简：截图直接进入剪贴板，没有多余步骤。
- 高级：截图后在原选区直接打开编辑界面和功能栏，按需提供马赛克、聚焦输入的 `T` 文字工具、本地 OCR、矩形和圆形标记、直线和箭头线，以及一键下载功能。选区支持二次拖拽编辑：向内拖动边框是裁剪，向外拖动会重新截取多出来的屏幕区域。滚动长图保持选区宽度，可在不缩小图片的情况下纵向浏览。

滚动截图时，点击选框右下角的**完成**按钮或按回车键即可拼接完成。

Clip 会跟随 macOS 的第一首选语言：简体中文使用简体中文界面；英语、繁体中文、日语、韩语和其他语言使用英文界面。修改 macOS 的语言偏好并重新启动 Clip 后生效。
